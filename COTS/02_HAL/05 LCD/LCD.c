...
void LCD_init( ){
	...
	DIO_voidSetPinDirection(DATA_PORT ,PIN4 , GPIO_OUTPUT_2MHZ_PP) ;
	DIO_voidSetPinDirection(DATA_PORT ,PIN5 , GPIO_OUTPUT_2MHZ_PP) ;
	DIO_voidSetPinDirection(DATA_PORT ,PIN6 , GPIO_OUTPUT_2MHZ_PP) ;
	DIO_voidSetPinDirection(DATA_PORT ,PIN7 , GPIO_OUTPUT_2MHZ_PP) ;
	_delay_ms(1000) ;
	LCD_writecmd(0X33) ;
	LCD_writecmd(0X32) ;
	LCD_writecmd(0x28) ;
...
void LCD_writechar(u8 data) {
		...
		DIO_voidSethlafPortval(DATA_PORT, LOWER_half ,upp_nibb(0x00 ,data)    ) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
		_delay_ms(10) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN ,GPIO_LOW) ;
		_delay_ms(10) ;
		DIO_voidSethlafPortval(DATA_PORT,LOWER_half , low_nibb(0x00 ,data)    ) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
		_delay_ms(10) ;
		DIO_voidSetPinValue(CONTROL_PORT,EN ,GPIO_LOW) ;
		_delay_ms(10) ;
...
void LCD_writecmd(u8 cmd)  {
	...
	DIO_voidSethlafPortval(DATA_PORT,LOWER_half,upp_nibb(0x00,cmd)     ) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
	_delay_ms(10) ;
	...
	DIO_voidSethlafPortval(DATA_PORT,LOWER_half,low_nibb(0x00,cmd)     ) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN , GPIO_HIGH);
	_delay_ms(10) ;
	DIO_voidSetPinValue(CONTROL_PORT,EN ,GPIO_LOW) ;
	_delay_ms(10) ;
